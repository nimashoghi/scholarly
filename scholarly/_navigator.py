...
from .data_types import Author, ProxyMode, Publication, PublicationSource
...
class Navigator(object, metaclass=Singleton):
    ...
    def use_proxy(self, pg1: ProxyGenerator, pg2: ProxyGenerator = None):
        ...
        if pg2 is not None:
            ...
            if not proxy_works:
                self.logger.info(
                    "FreeProxy as a secondary proxy is not working. "
    ...
    def _get_page(self, pagerequest: str, premium: bool = False) -> str:
        ...
        while tries < self._max_retries:
            try:
                ...
                if (
                    premium is False
                ):  # premium methods may contain sensitive information
    ...
    def search_publication(self, url: str, filled: bool = False) -> Publication:
        ...
        pub = publication_parser.get_publication(
            soup.find_all("div", "gs_or")[0],
