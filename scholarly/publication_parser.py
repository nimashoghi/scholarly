...
from .data_types import Mandate, Publication, PublicationSource
...
class _SearchScholarIterator(object):
    ...
    def _load_url(self, url: str):
        ...
        self._pos = 0
        self._rows = (
            self._soup.select("div.gs_r.gs_or.gs_scl")
            + self._soup.select("div.gs_r.gs_or.gs_scl.gs_fmar")
            + self._soup.select("div.gsc_mpat_ttl")
        )
...
class PublicationParser(object):
    ...
    def fill(self, publication: Publication) -> Publication:
        ...
        if publication["source"] == PublicationSource.AUTHOR_PUBLICATION_ENTRY:
            ...
            parsed_bib = bibtexparser.parse_string(bibtex).entries[-1]
    ...
    def bibtex(self, publication: Publication) -> str:
        ...
        converted_dict = converted_dict
        library.add(converted_dict)
